...
async function backup() {
    ...
    try {
        ...
        await fs.writeFile('product_prices_backup_id7.json', JSON.stringify(rows, null, 2));
        console.log(`Backup saved to product_prices_backup_id7.json (${(rows as any[]).length} records)`);
    } catch (err) {
